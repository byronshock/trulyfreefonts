...
**Who:** Claude; the owner picks the sample text. **Depends on:** 1, 2; M2-D5; D3 (`preview_ok`); M1 steps 6b (each family's `font_file` {url, sha256}) and 18. Items follow M2-D5 (a); the last is for (b).
...
| **M2-D7 [Step 0]: a test site before launch** | **(a) `staging.trulyfreefonts.com`** (step 11): round 1 runs there and Milestone 3 reuses it; (b) none: round 1 uses a laptop build shared on screen, and everything else is first tried live. |
